...
| Contracts and providers | Inactive contract-template registry, hashed artifacts, counsel/owner activation gates, provider readiness checks, webhook HMAC and idempotency receipts | Contract/provider, webhook security, and migration tests | Safe boundary complete; no legal template, state policy, or provider is activated |
...
2. Contract templates require user-supplied artifacts plus matching-state counsel and owner approval. Placeholder records cannot activate or execute.
