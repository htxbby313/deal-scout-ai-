...
export async function createCampaignAction(_state: PipelineActionState, data: FormData) {
  await requireOwner();
  return result(() => createCampaignRecord({ name: text(data, "name"), state: text(data, "state"), type: text(data, "type") as "SELLER_ACQUISITION" | "BUYER_DISPOSITION", actor: "owner", counties: text(data, "counties"), cities: text(data, "cities"), zipCodes: text(data, "zipCodes"), neighborhoods: text(data, "neighborhoods"), includedPropertyTypes: text(data, "includedPropertyTypes"), excludedPropertyTypes: text(data, "excludedPropertyTypes"), acquisitionStrategy: text(data, "acquisitionStrategy"), targetBuyerGroup: text(data, "targetBuyerGroup"), minimumRequiredProfitCents: cents(data, "minimumRequiredProfit"), maximumEarnestMoneyCents: cents(data, "maximumEarnestMoney"), maximumResearchCostCents: cents(data, "maximumResearchCost"), maximumOutreachCostCents: cents(data, "maximumOutreachCost"), evidenceFreshnessHours: number(data, "evidenceFreshnessHours"), startsAt: date(data, "startsAt"), endsAt: date(data, "endsAt"), audienceCriteria: { description: text(data, "audienceCriteria") }, sourceRequirements: { description: text(data, "sourceRequirements") } }), "Campaign boundary saved in draft with outbound disabled.");
}
