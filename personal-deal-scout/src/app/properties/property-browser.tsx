...
function ResearchPanel({ property }: { property: PropertyView }) {
  ...
  return <section className="mt-7 border-t pt-6">
    ...
    {property.researchRuns[0] ? <p className="mt-3 rounded-lg bg-slate-50 p-3 text-xs text-slate-600">Latest run: {property.researchRuns[0].findingsFound} verified · {property.researchRuns[0].manualNeeded} manual · {property.researchRuns[0].sourcesChecked} sources checked</p> : <p className="mt-3 rounded-lg bg-amber-50 p-3 text-xs text-amber-900">Not researched yet. Run the dossier before sending this property to a developer.</p>}
    ...
    {manual ? <p className="mt-3 text-xs font-semibold text-amber-800">{manual} research topic{manual === 1 ? "" : "s"} require human follow-up.</p> : null}
  </section>;
}
...
export function PropertyBrowser({ properties }: { properties: PropertyView[] }) {
  ...
  const [sort, setSort] = useState("research-desc");
  ...
  return <>
    <div className="rounded-2xl border bg-white p-4 shadow-sm"><div className="grid gap-3 lg:grid-cols-[minmax(220px,1fr)_repeat(4,minmax(130px,auto))]"><input className="min-w-0 rounded-xl border bg-slate-50 px-4 py-2.5 text-sm" onChange={(event) => setQuery(event.target.value)} placeholder="Search address, region, contact, source…" value={query} /><select aria-label="State" className="rounded-xl border px-3 py-2 text-sm" onChange={(event) => { setState(event.target.value); setCounty(""); setNeighborhood(""); }} value={state}><option value="">Nation · all states</option>{states.map((item) => <option key={item}>{item}</option>)}</select><select aria-label="County" className="rounded-xl border px-3 py-2 text-sm" onChange={(event) => { setCounty(event.target.value); setNeighborhood(""); }} value={county}><option value="">All counties</option>{counties.map((item) => <option key={item}>{item}</option>)}</select><select aria-label="Neighborhood" className="rounded-xl border px-3 py-2 text-sm" onChange={(event) => setNeighborhood(event.target.value)} value={neighborhood}><option value="">All neighborhoods</option>{neighborhoods.map((item) => <option key={item}>{item}</option>)}</select><select aria-label="Sort properties" className="rounded-xl border px-3 py-2 text-sm" onChange={(event) => setSort(event.target.value)} value={sort}><option value="research-desc">Most researched</option><option value="confidence">Highest confidence</option><option value="price-asc">Price low to high</option><option value="price-desc">Price high to low</option><option value="address">Address A–Z</option></select></div><div className="mt-3 flex rounded-xl border p-1 sm:w-fit"><button className={`rounded-lg px-3 py-1.5 text-sm font-semibold ${view === "actionable" ? "bg-slate-950 text-white" : "text-slate-500"}`} onClick={() => setView("actionable")}>Actionable · {actionableCount}</button><button className={`rounded-lg px-3 py-1.5 text-sm font-semibold ${view === "research" ? "bg-amber-600 text-white" : "text-slate-500"}`} onClick={() => setView("research")}>Needs verification · {properties.length - actionableCount}</button></div><p className="mt-3 text-xs text-slate-500">Nation → {state || "state"} → {county || "county"} → {neighborhood || "neighborhood"} → {filtered.length} address{filtered.length === 1 ? "" : "es"}</p></div>
    <div className="mt-5"><PropertyMap onSelect={setSelectedId} properties={filtered} /></div>
    <div className="mt-5 grid gap-5 md:grid-cols-2 2xl:grid-cols-3">{filtered.map((property) => <button className="overflow-hidden rounded-2xl border bg-white text-left shadow-sm transition hover:-translate-y-0.5 hover:shadow-md" key={property.id} onClick={() => setSelectedId(property.id)}><div className="relative grid min-h-40 place-items-center overflow-hidden bg-slate-100">{property.media[0] ? <Image alt={property.media[0].altText} className="object-cover" fill loader={sourceImageLoader} referrerPolicy="no-referrer" sizes="(min-width: 1536px) 33vw, (min-width: 768px) 50vw, 100vw" src={property.media[0].url} unoptimized /> : <><span className="text-5xl opacity-40">⌂</span><span className="absolute bottom-3 right-3 rounded-full bg-amber-100 px-3 py-1 text-xs font-bold text-amber-900">Photo needs verification</span></>}<span className="absolute left-3 top-3 rounded-full bg-white/90 px-3 py-1 text-xs font-bold">{labels[property.opportunityStatus]}</span></div><div className="p-5"><div className="flex justify-between gap-3"><div><h2 className="font-bold">{property.address}</h2><p className="mt-1 text-sm text-slate-500">{property.city}, {property.state} {property.zipCode}</p></div><span className="h-fit rounded-full bg-blue-50 px-2.5 py-1 text-xs font-bold text-blue-800">{property.matches.length} matches</span></div><div className="mt-4 grid grid-cols-2 gap-2 text-xs"><span className="rounded-lg bg-slate-50 p-2"><span className="block text-slate-500">Value</span><b>{money(property.estimatedValue)}</b></span><span className="rounded-lg bg-slate-50 p-2"><span className="block text-slate-500">Research</span><b>{property.researchFindings.filter((item) => item.status === "VERIFIED").length}/12 verified</b></span></div><p className="mt-4 text-sm font-semibold text-blue-700">Open dossier →</p></div></button>)}</div>
