...
export async function GET(request: NextRequest) {
  ...
  const result = serializeOperationalCsv(
    report.metrics,
    filters,
    report.generatedAt,
    operationalScopeParts(
  ...
  await getPrisma().auditLog.create({
    data: {
      type: "executive.kpi_exported",
      summary: `Exported ${result.rowCount} operational KPI rows.`,
      details: {
  ...
  return new Response(result.csv, {
    headers: {
      ...
      "content-disposition": `attachment; filename="deal-scout-executive-${report.generatedAt.slice(0, 10)}.csv"`,
