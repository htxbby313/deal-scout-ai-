...
export async function POST(request: Request) {
  ...
  if (!verifyWebhookSignature(body, request.headers.get("x-deal-scout-signature"), resolveIntegrationEnvironment().webhookSecret)) return Response.json({ ok: false, error: "Webhook verification failed" }, { status: 401 });
