...
export default async function MessageCenterPage({
  ...
  const pathwayDrafts = drafts.filter((draft) =>
    pathway === "DEVELOPER_BUYER_ACQUISITION"
        ...
        <section className="space-y-4">
          <div className="rounded-2xl border bg-white p-5">
            <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-[0.14em] text-slate-400">
              <span>{isDeveloperPathway ? "Company relationship" : "Property relationship"}</span>
            </div>
            <h2 className="mt-2 text-lg font-bold">{pathwayMeta.label}</h2>
            <p className="mt-1 text-sm text-slate-500">{pathwayMeta.description}</p>
