...
export function FinancialProjectionForm({ transactions }: { transactions: Array<{ id: string; label: string }> }) {
  ...
  return <form action={action} className="mt-4 grid gap-3 md:grid-cols-4">
    ...
    <textarea className={`${input} md:col-span-3`} name="evidenceNotes" placeholder="Pricing assumptions, cost sources, and unresolved items" required /><Field name="correctionReason" placeholder="Correction reason for later versions" />
...
export function SettlementReviewForm({ transactions }: { transactions: Array<{ id: string; label: string }> }) {
  ...
  return <form action={action} className="mt-4 grid gap-3 md:grid-cols-3"><select className={input} name="transactionId" required><option value="">Transaction</option>{transactions.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</select><Field name="grossAssignmentFee" placeholder="Actual gross proceeds" required type="number" /><Field name="actualExpenses" placeholder="Actual itemized expenses total" required type="number" /><Field name="settlementDocumentUrl" placeholder="Settlement document HTTPS URL" required type="url" /><Field name="settlementDocumentHash" placeholder="Settlement document SHA-256" required /><Field name="reviewedAt" placeholder="Reviewed at" required type="datetime-local" /><Field name="correctionReason" placeholder="Correction reason for later versions" /><button className="rounded-lg bg-emerald-800 px-4 py-2 text-sm font-bold text-white disabled:opacity-50 md:col-span-2" disabled={pending}>{pending ? "Recording…" : "Record reviewed closed profit"}</button><Result state={state} /></form>;
}
