...
import { AGENT_TASK_TYPES, evaluateAgentTask, evaluateSupervisedTrackRecord, taskActionPolicy, taskOwner, type AgentTaskType } from "@/lib/agent-workflow-policy";
...
export async function ensureAgentTeam() {
  ...
  for (const taskType of AGENT_TASK_TYPES) {
    const policy = taskActionPolicy[taskType]; const agent = byRole.get(taskOwner[taskType]); if (!agent) continue;
    const mode = policy.zone === "GREEN" ? "AUTOMATIC_INTERNAL" : policy.zone === "YELLOW" ? "APPROVAL_REQUIRED" : "BLOCKED";
    await db.agentCapabilityGrant.upsert({ where: { agentId_capability_jurisdictionState_channel: { agentId: agent.id, capability: policy.capability, jurisdictionState: "*", channel: "*" } }, update: { mode }, create: { agentId: agent.id, capability: policy.capability, mode, jurisdictionState: "*", channel: "*", maximumCostCents: 0, minimumEvidenceCount: policy.zone === "GREEN" ? 0 : 1 } });
  }
  return agents;
}

export async function createTaskIfMissing(input: { role: AgentRole; taskType: AgentTaskType; title: string; description: string; transactionId?: string; propertyId?: string; developerId?: string; evidenceCount?: number; ownerApprovalRequired?: boolean; expectedValueCents?: bigint | null; expectedBenefit?: string; materialRisks?: string[] }) {
  const db = getPrisma();
...
export async function seedAgentWork() {
  ...
  let propertiesConsidered = 0; let developersConsidered = 0; let transactionsConsidered = 0;
  ...
  while (true) {
    const page = await db.property.findMany({ where: { opportunityStatus: { not: "REJECTED" } }, include: { researchFindings: true }, orderBy: { id: "asc" }, take: 100, ...(propertyCursor ? { cursor: { id: propertyCursor }, skip: 1 } : {}) });
    for (const property of page) {
      propertiesConsidered += 1;
      const evidenceCount = property.researchFindings.filter((item) => item.status === "VERIFIED").length;
      await createTaskIfMissing({ role: "RESEARCH", taskType: "RESEARCH_PROPERTY", title: `Refresh ${property.address}`, description: "Refresh configured public sources and preserve manual-verification gaps.", propertyId: property.id, evidenceCount, ownerApprovalRequired: false });
    }
    if (page.length < 100) break; propertyCursor = page.at(-1)!.id;
  }
  let developerCursor: string | undefined;
  while (true) {
    const page = await db.developer.findMany({ where: { active: true }, orderBy: { id: "asc" }, take: 100, ...(developerCursor ? { cursor: { id: developerCursor }, skip: 1 } : {}) });
    for (const developer of page) { developersConsidered += 1; await createTaskIfMissing({ role: "RESEARCH", taskType: "RESEARCH_DEVELOPER", title: `Refresh ${developer.companyName}`, description: "Refresh official public-source developer evidence.", developerId: developer.id, evidenceCount: developer.contactVerifiedAt ? 1 : 0, ownerApprovalRequired: false }); }
    if (page.length < 100) break; developerCursor = page.at(-1)!.id;
  }
  let transactionCursor: string | undefined;
  while (true) {
    const page = await db.dealTransaction.findMany({ where: { controlStatus: { not: "STOPPED" } }, include: { property: true, documents: true, approvals: true }, orderBy: { id: "asc" }, take: 100, ...(transactionCursor ? { cursor: { id: transactionCursor }, skip: 1 } : {}) });
    for (const transaction of page) {
      transactionsConsidered += 1;
      await createTaskIfMissing({ role: "TRANSACTION_COMPLIANCE", taskType: "REVIEW_COMPLIANCE_EVIDENCE", title: `Review controls · ${transaction.property.address}`, description: "List missing documents, approvals, counsel, and compliance evidence.", transactionId: transaction.id, propertyId: transaction.propertyId, evidenceCount: transaction.documents.length + transaction.approvals.length, ownerApprovalRequired: false });
    }
    if (page.length < 100) break; transactionCursor = page.at(-1)!.id;
  }
  return { properties: propertiesConsidered, developers: developersConsidered, transactions: transactionsConsidered };
}
...
async function performTask(task: Awaited<ReturnType<typeof loadTask>>) {
  ...
  if (taskType === "RESEARCH_PROPERTY" && task.propertyId) {
    const current = await getPrisma().propertyResearchRun.findFirst({ where: { propertyId: task.propertyId, status: { in: ["COMPLETE", "NEEDS_MANUAL_VERIFICATION"] }, finishedAt: { gte: new Date(Date.now() - 7 * 86_400_000) } }, orderBy: { finishedAt: "desc" } });
  ...
  if (taskType === "UNDERWRITE_PROFIT" && task.propertyId) {
    ...
    const [property, funnel, match] = await Promise.all([
      db.property.findUniqueOrThrow({ where: { id: task.propertyId }, include: { researchFindings: true, transactions: { include: { financialProjections: { orderBy: { version: "desc" }, take: 1 } }, orderBy: { createdAt: "desc" }, take: 1 } } }),
      db.acquisitionFunnel.findFirst({ where: { propertyId: task.propertyId }, include: { buyerPriceEvidence: { orderBy: { createdAt: "desc" }, take: 1 } }, orderBy: { createdAt: "desc" } }),
      db.developerMatch.findFirst({ where: { propertyId: task.propertyId }, orderBy: { score: "desc" } }),
    ]);
    const projection = property.transactions[0]?.financialProjections[0] ?? null;
    const buyer = funnel?.buyerPriceEvidence[0] ?? null;
    const knownCosts = projection ? projection.transactionCostsCents + projection.doubleClosingCostsCents + projection.titleExpensesCents + projection.closingExpensesCents + projection.transactionalFundingCents + projection.financingCostsCents + projection.taxesCents + projection.liensAndPayoffsCents + projection.concessionsCents + projection.inspectionExpensesCents + projection.legalExpensesCents + projection.dataMarketingCostsCents + projection.insuranceExpensesCents + projection.otherExpensesCents + projection.contingencyReserveCents : null;
    const result = underwriteAgentOpportunity({ sellerPriceCents: projection?.sellerContractPriceCents ?? (property.estimatedValue ? BigInt(property.estimatedValue) * BigInt(100) : null), sellerPriceVerified: Boolean(projection || (property.estimatedValue && property.verificationSourceUrl)), buyerLowCents: projection?.buyerPriceLowCents ?? buyer?.lowCents ?? null, buyerBaseCents: projection?.buyerPriceBaseCents ?? buyer?.baseCents ?? null, buyerHighCents: projection?.buyerPriceHighCents ?? buyer?.highCents ?? null, buyerPriceStatus: projection?.buyerPriceStatus ?? buyer?.status ?? null, buyerPriceCurrent: Boolean(projection ? projection.buyerPriceExpiresAt > new Date() : buyer && buyer.expiresAt > new Date()), knownCostsCents: knownCosts, riskReserveCents: projection?.riskReserveCents ?? null, evidenceCount: property.researchFindings.filter((finding) => finding.status === "VERIFIED").length, buyerMatchScore: match?.score ?? null });
    return { summary: result.ready ? `Evidence-backed profit underwriting completed with a ${result.score ?? 0}/100 priority score.` : `${result.blockers.length} underwriting evidence gaps require research.`, output: result };
  }
...
async function chainCompletedAgentTask(task: NonNullable<Awaited<ReturnType<typeof loadTask>>>, output: unknown) {
  if (!task.propertyId) return;
  const db = getPrisma();
  const property = await db.property.findUnique({ where: { id: task.propertyId }, select: { address: true, researchFindings: { where: { status: "VERIFIED" }, select: { id: true } } } });
  if (!property) return;
  ...
  if (["ASSESS_SELLER_FIT", "MATCH_BUYER"].includes(task.taskType)) {
    const completed = await db.agentTask.findMany({ where: { propertyId: task.propertyId, taskType: { in: ["ASSESS_SELLER_FIT", "MATCH_BUYER"] }, status: "COMPLETED" }, select: { taskType: true } });
    if (new Set(completed.map((item) => item.taskType)).size === 2) await createTaskIfMissing({ role: "PROFIT_UNDERWRITING", taskType: "UNDERWRITE_PROFIT", title: `Underwrite profit · ${property.address}`, description: "Calculate only evidence-backed, non-guaranteed profit and identify missing financial facts.", transactionId: task.transactionId ?? undefined, propertyId: task.propertyId, evidenceCount, ownerApprovalRequired: false });
  }
  if (task.taskType === "UNDERWRITE_PROFIT") {
    const result = output as { ready?: boolean; projected?: { feeBaseCents?: string | null }; blockers?: string[] };
    if (result.ready) await createTaskIfMissing({ role: "COMMUNICATIONS_DISPOSITION", taskType: "PREPARE_ACTION_PACKAGE", title: `Prepare next action · ${property.address}`, description: "Prepare the exact action, evidence, expected benefit, cost, and risks for owner review.", transactionId: task.transactionId ?? undefined, propertyId: task.propertyId, evidenceCount, ownerApprovalRequired: false, expectedValueCents: result.projected?.feeBaseCents ? BigInt(result.projected.feeBaseCents) : null, expectedBenefit: "Advance a verified opportunity toward an approved seller or buyer conversation.", materialRisks: result.blockers ?? [] });
  }
  if (task.taskType === "PREPARE_ACTION_PACKAGE") await createTaskIfMissing({ role: "OPERATIONS_COORDINATOR", taskType: "REQUEST_OWNER_APPROVAL", title: `Decision ready · ${property.address}`, description: "Review one specific next action. Approval does not authorize a campaign or unrelated contact.", transactionId: task.transactionId ?? undefined, propertyId: task.propertyId, evidenceCount, ownerApprovalRequired: true, expectedValueCents: task.expectedValueCents, expectedBenefit: task.expectedBenefit ?? undefined, materialRisks: task.materialRisks });
}

async function loadTask(taskId: string) {
  return getPrisma().agentTask.findUnique({ where: { id: taskId }, include: { assignedAgent: { include: { capabilityGrants: true } }, transaction: { include: { documents: true, approvals: true } } } });
}
...
export async function runAgentQueue(agentId: string, limit = 3, seed = true) {
  ...
  const tasks = await getPrisma().agentTask.findMany({ where: { assignedAgentId: agentId, status: "QUEUED" }, orderBy: [{ priority: "desc" }, { createdAt: "asc" }], take: Math.max(1, Math.min(limit, 10)) });
  const results = [];
...
export async function runAgentTeamBatch() {
  ...
  await seedAgentWork();
  const results = [];
  const order: AgentRole[] = ["RESEARCH", "SELLER_ACQUISITION", "BUYER_DEVELOPER", "PROFIT_UNDERWRITING", "COMMUNICATIONS_DISPOSITION", "TRANSACTION_COMPLIANCE", "OPERATIONS_COORDINATOR"];
  for (let wave = 0; wave < 3; wave += 1) for (const role of order) {
    const agent = agents.find((item) => item.role === role && item.status === "ACTIVE");
    if (agent) results.push({ wave, agentId: agent.id, ...(await runAgentQueue(agent.id, 10, false)) });
  }
...
export async function readAgentDashboard() {
  await ensureAgentTeam();
  const db = getPrisma();
  const [agents, approvalTasks, recentTasks, events, scheduler] = await Promise.all([
  ...
  const taskSummary = (task: (typeof recentTasks)[number]) => ({ id: task.id, title: task.title, agentId: task.assignedAgentId, agentName: task.assignedAgent.name, status: task.status, transactionLabel: task.transaction?.property.address ?? null, evidenceCount: task.evidenceCount, actionZone: task.actionZone, capability: task.capability, estimatedCostCents: task.estimatedCostCents.toString(), expectedValueCents: task.expectedValueCents?.toString() ?? null, expectedBenefit: task.expectedBenefit, materialRisks: task.materialRisks, createdAt: task.createdAt.toISOString(), updatedAt: task.updatedAt.toISOString() });
  return {
    scheduler,
