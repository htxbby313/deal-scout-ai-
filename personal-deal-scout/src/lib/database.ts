...
export type PropertyResearchFindingRecord = { id: string; propertyId: string; topic: string; label: string; value?: string; status: "VERIFIED" | "NOT_FOUND" | "CONFLICT" | "NEEDS_MANUAL_VERIFICATION"; sourceName?: string; sourceUrl?: string; observedAt: string; confidence: number; notes?: string };
export type PropertyMediaRecord = { id: string; propertyId: string; url: string; sourceUrl: string; sourceName: string; caption?: string; altText: string; kind: "LISTING_PHOTO" | "MAP" | "PARCEL" | "DOCUMENT" | "OTHER"; position: number; sendApproved: boolean; discoveredAt: string; reviewedAt?: string };
export type PropertyResearchRunRecord = { id: string; propertyId: string; status: string; sourcesChecked: number; findingsFound: number; manualNeeded: number; error?: string; startedAt: string; finishedAt?: string };
export type PropertyRecord = { id: string; address: string; city: string; state: string; zipCode: string; ownerName: string; county?: string; neighborhood?: string; latitude?: number; longitude?: number; marketFips?: string; yearBuilt?: string; lotSize?: string; estimatedValue?: number; notes?: string; opportunityStatus: OpportunityStatus; contactName?: string; contactPhone?: string; contactEmail?: string; contactUrl?: string; sourceName?: string; sourceUrl?: string; sourceRecordDate?: string; verificationSourceUrl?: string; verificationDate?: string; lastVerifiedAt?: string; confidence: number; createdAt: string; updatedAt: string; researchFindings: PropertyResearchFindingRecord[]; media: PropertyMediaRecord[]; researchRuns: PropertyResearchRunRecord[] };
...
export async function createProperty(input: z.infer<typeof propertyInputSchema>) {
  try {
    ...
    return await getPrisma().$transaction(async (tx) => {
      ...
      await audit(tx, "property.created", `Created property ${property.address}.`, { propertyId: property.id, marketFips: property.marketFips, sourceUrl: property.sourceUrl });
      return property;
    });
