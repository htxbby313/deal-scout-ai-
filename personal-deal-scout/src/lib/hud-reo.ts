  ...
  try {
    ...
    const result = await db.$transaction(async (tx) => {
      ...
      for (const record of countyListings) {
        ...
        const evidence = { city: record.CITY.trim(), state: record.STATE_CODE, ownerName: "U.S. Department of Housing and Urban Development", marketFips: fips, opportunityStatus: "GOVERNMENT_SALE" as const, sourceName: HUD_REO_SOURCE, sourceUrl: hudRecordSourceUrl(record.OBJECTID), sourceRecordDate: dateFromEpoch(record.DATE_ACQUIRED), lastVerifiedAt: capturedAt, confidence: 90, notes: `HUD FHA case ${record.CASE_NUM}. Public listing status step 6 observed ${capturedAt.toISOString().slice(0, 10)}. HUD acquisition date is stored as the source record date; asking price and seller contact were not published in this layer.` };
