...
export async function readOperatingLayer(filters: { stage?: string; status?: string } = {}) {
  ...
  return {
    ...
    buyerDemand: buyerDemand.map((demand) => ({ id: demand.id, developer: demand.developer.companyName, version: demand.version, status: demand.status, markets: [...demand.states, ...demand.counties, ...demand.zipCodes], expiresAt: demand.expiresAt?.toISOString() ?? null, sourceUrl: demand.sourceUrl })),
...
export async function createBuyerDemandRecord(input: { developerId: string; actor: string; states: string; counties: string; zipCodes: string; assetTypes: string; minPurchasePriceCents?: bigint; maxPurchasePriceCents?: bigint; maxAssignmentFeeCents?: bigint; strategy?: string; sourceUrl: string; expiresAt: Date }) {
  ...
  return getPrisma().$transaction(async (tx) => {
    ...
    return tx.buyerDemandVersion.create({ data: { developerId: input.developerId, version: (latest?.version ?? 0) + 1, status: "DRAFT", states: list(input.states).map((item) => item.toUpperCase()), counties: list(input.counties), zipCodes: list(input.zipCodes), assetTypes: list(input.assetTypes), minPurchasePriceCents: input.minPurchasePriceCents, maxPurchasePriceCents: input.maxPurchasePriceCents, maxAssignmentFeeCents: input.maxAssignmentFeeCents, strategy: input.strategy, sourceUrl: httpsUrl(input.sourceUrl), expiresAt: input.expiresAt, createdBy: input.actor } });
  }, { isolationLevel: Prisma.TransactionIsolationLevel.Serializable });
}

export async function createCampaignRecord(input: { name: string; state: string; type: "SELLER_ACQUISITION" | "BUYER_DISPOSITION"; actor: string; minimumRequiredProfitCents?: bigint; maximumEarnestMoneyCents?: bigint; startsAt: Date; endsAt: Date; audienceCriteria: Prisma.InputJsonValue; sourceRequirements: Prisma.InputJsonValue }) {
