...
export async function createStagePolicy(input: { stage: AcquisitionStage; reviewIntervalHours: number; expiryAction: "REFRESH_RESEARCH" | "MANUAL_VERIFICATION" | "NURTURE" | "DISQUALIFY" | "ARCHIVE"; requiredGateTypes: Array<"PROPERTY_EVIDENCE" | "SELLER_CONTACT" | "UNDERWRITING" | "COMPLIANCE" | "CONTRACT" | "BUYER_COVERAGE" | "DISPOSITION" | "CLOSING">; highValueThresholdCents?: bigint; reason: string; actor: string; effectiveAt?: Date; expiresAt?: Date }) {
...
export async function runFunnelExpirationCycle(now = new Date()) {
  ...
  for (const funnel of funnels) {
    ...
    await db.$transaction(async (tx) => {
      ...
      if (decision.action === "REFRESH_RESEARCH") { const active = await tx.propertyResearchRun.findFirst({ where: { propertyId: funnel.propertyId, status: { in: ["QUEUED","RUNNING"] } } }); if (!active) await tx.propertyResearchRun.create({ data: { propertyId: funnel.propertyId, status: "QUEUED" } }); refreshed += 1; }
