...
export type OwnerQueueItem = { id: string; kind: "AGENT_TASK" | "TRANSACTION_APPROVAL" | "FUNNEL_BLOCKER" | "SELLER_ENGAGEMENT" | "CONTRACT_TEMPLATE"; label: string; detail?: string; createdAt: Date; urgent: boolean; href: string };
...
export async function readFunnelOwnerQueue() {
  const db = getPrisma();
  const [agentTasks, approvals, blockers, engagements, templates] = await Promise.all([
    db.agentTask.findMany({ where: { status: "WAITING_FOR_APPROVAL", ownerApprovalRequired: true }, include: { assignedAgent: true, transaction: { include: { property: true } } }, orderBy: { updatedAt: "asc" }, take: 100 }),
  ...
  return sortOwnerQueue([
    ...agentTasks.map((item) => ({ id: item.id, kind: "AGENT_TASK" as const, label: item.title, detail: `${item.assignedAgent.name}${item.transaction?.property.address ? ` · ${item.transaction.property.address}` : ""} · ${item.evidenceCount} evidence item${item.evidenceCount === 1 ? "" : "s"}`, createdAt: item.updatedAt, urgent: item.priority === "URGENT", href: `/agents#task-${item.id}` })),
...
export async function readOwnerAgentActivity() {
  const events = await getPrisma().agentEvent.findMany({ include: { actorAgent: true, task: { include: { assignedAgent: true } } }, orderBy: { createdAt: "desc" }, take: 8 });
  return events.map((event) => ({ id: event.id, agentName: event.actorAgent?.name ?? event.task.assignedAgent.name, summary: event.summary, createdAt: event.createdAt }));
}
